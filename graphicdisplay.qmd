...
Visualizing Earth observation data makes complex numbers and statistics much easier to understand than raw tables. Good graphics help us see patterns, similarities and relationships in multidimensional geospatial data that might otherwise stay hidden. They are a core tool in exploratory data analysis: visualization can reveal outliers, clusters, local trends, boundaries or missing values, and thus support data cleaning and pre-processing, especiaally in multi-band raster data and temporal stacks such as data cubes. At the same time, graphics are not a substitute for careful analysis. They should be read critically and always in context, considering data sourrces, collection methods and design choices, so that they stimulate new questions and insights without leading us to quicl or misleading conclusions.
