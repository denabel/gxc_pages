...
<details>
  ...
  To understand how Earth Observation works, it helps to first look at the basics of remote sensing. A key idea here is **electromagnetic radiation**. This is energy that travels from an objects (for example the Earth’s surface) through space in the form of waves with different wavelengths and frequencies. Remote sensing sensors detect this energy and turn it into data we can analyse. Because different objects interact with this energy in different ways, we can learn a lot about them, for example, how healthy crops are, how much water is in lakes and soils, where forests are being cut down, how cities are expanding, or where surfaces are unusually hot during heat waves. 
An important concept in this process is **radiance**. Radiance describes how much electromagnetic radiation an object emits or reflects in a specific direction. You can think of it as the amount of „signal“ coming from a target that actually reaches the sensor. In contrast, **irradiance** is the radiation that arrives at the surface of the object in the first place. 
Closely related is **reflectance**. Reflectance tells us how much of the incoming radiation a surface sens back compared to oh much it receives (irradiance – radiance). Different materials absorb and reflect different parts of the spectrum, so each has its own **spectral signature** – a kind of optical fingerprint across many wavelengths. This is what allows us to distinguish, for example, vegetation, water and soil in satellite images. 
Healthy vegetation reflects a lot of green light (which is why plants look green) and also a lot of near-infrared (NIR) light, while strongly absorbing red and blue light for photosynthesis. Water, on the other hand, reflects relatively little overall: most radiation is absorbed or passes into the water. Shorter wavelengths like blue and green are reflected a bit more, which is why water often appears blue. Soil reflectance depends strongly on moisture; wet soil tends to be darker and reflects less light, whereas dry soil is brighter and reflects more. 
Together, these concepts explain how we get from energy interacting with objects on the Earth’s surface to Earth Observation imagery and, finally, to information about surface properties and environmental conditions. 
