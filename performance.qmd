...
ggplot(samples) +
  ...
  facet_wrap(~sample, ncol = 3, dir = "v", labeller = function(variable, value) {
    paste("Sample =", gsub("sample_", "", value))
  }) +
  theme_void()
```

To analyse performance, we will run the following R code repeatedly:
...
We use the `2m_temperature` indicator from the [`ERA5-Land` monthly averages](https://doi.org/10.24381/cds.68d2bb30) for this exercise. The code was run on an average office laptop (Intel Core i7-10510U CPU, 16GB RAM, Windows 10). The benchmarking is powered by the [bench](https://bench.r-lib.org/) package. To see the replication code, expand the collapsed code below.

<details><summary>Code for benchmarking</summary>
```{r execute-time2}
...
make_link_expr <- function(.data, ts) {
  bquote(
    gxc::link_monthly(
      .data = .(.data),
      indicator = "2m_temperature",
...
link_exprs <- lapply(samples_list, function(sample) {
  time_span_values <- c(0, 2, 11)
  lapply(time_span_values, function(time_span) {
      make_link_expr(sample, time_span)
  })
}) |>
  unlist(recursive = FALSE)

bm <- do.call(bench::mark, c(link_exprs, iterations = 1, check = FALSE))
```
</details>
