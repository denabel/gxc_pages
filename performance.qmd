...
bm$time_span <- sapply(as.character(bm$time_span), switch, `0` = "month", `2` = "seasonal", `11` = "yearly")

plots <- lapply(c("ger", "eu"), function(extent) {
  data <- bm[bm$extent %in% extent, ]
  p <- ggplot(data, aes(x = sample_size, y = median, color = time_span)) +
    geom_point() +
  ...
  ggplotly(p) |>
    layout(visible = identical(extent, "ger"))
}) |>
  setNames(c("ger", "eu"))

n1 <- length(plots$ger$x$data)
n2 <- length(plots$eu$x$data)

vis_eu <- c(rep(TRUE, n1), rep(FALSE, n2))
vis_ger <- c(rep(FALSE, n1), rep(TRUE, n2))
