...
## Introduction to Nighttime Light (NL) Data
...
# acquisition/ history of acquisition
...
<details>
 <summary>Useful case studies and workflows</summary>

*Data*: GEOCON dataset & nighttime data DMSP-OLS or better VIIRS
*Pre-processing*: thresholding illumination (DN values), removing noise
*Disaggregation*: matching GEOCON and NLs reoslution)
_Further analysis_
([Chen, 2016](https://doi.org/10.1007/978-3-319-22810-5_15))

*Data*: acquisition of geo-coded Demographic and health Surveys from African counties
*Pre-processing*: buffering/ spatial analysis of data
*Data integration*: linking/ integration of nighttime lights data from National Oceanic and Atmospheric Administration (DMSP-OLS sensors)
_Statistical analysis_
([Bruderle, 2018](https://doi.org/10.1371/journal.pone.0202231))

*Data*: data from the India Lights Project API, including DMSP-OLS NL data from the National Oceanic and Atmospheric Administration (NOAA) and the 2011 Census of India, spatial framework is based on a shapefile of 2011 village boundaries and point data (village centroids) from ML InfoMap
*Pre-processing*: (“stable light” to reduce noise), shapefile of 2011 villages (ISO 9001-2015 actual village size and shape), define village centroids
*Data integration*: bilinear interpolation linking NLs data and centroids, calculating DN values in 2, 3, 5 km buffer around centroid
*Correlation analysis*: correlation coefficient (Pearson’s r) between NL ground truth data (i.e. Census)
*Regression & Evaluation*: multivariate regression accuracy of modelling ground data, testing for non-linearity 
([Dugoua, 2018](https://doi.org/10.1080/01431161.2017.1420936))

*Data*: annual composites of nighttime light data, Thematic World Borders Dataset for administrative boundaries (e.g., nation-states, territories), Socio-economic data sourced from World Bank (GDP, poverty headcount), UN, Eurostat, and national censuses (population), energy consumption (EIA) and emissions (CDIAC, EDGAR)
*Pre-processing*: thresholding (calculation of different light metrics, including “area lit”, “sum of lights,” “lights index,” and raw DN values), distinguish significance
*Correlation & regression analysis*: Pearsons's R for correlation of NLss and socio-economic variables, regression models to analyse spatial patterns
*Interpretation & Application*: NLs as a proxy for socio-economic data in countries with poor statistical coverage, spatial variability in night-time lights 
...
_model training with imputation logics_
...
## Possible indications for social sciences
...
NL data's accuracy is especially dependent on a stable electricity supply since there are difficulties with distinguishing the reason of variation in electricity supply. Also, bare in mind non-electrified villages are not detected (Dugoua, 2018). 
Obviously, a higher spatial resolutions leads to higher accuracy in capturing variations in NLs (Bruederle, 2018). 
Since NL data isn't a super accurate indicator for a lot of socioeconomic topics it can thus help formulate estimation especially in under-researched areas with poor statistical inventories (Proville, 2017) or difficult data access (Chen, 2016).  Talking about statistical inventories NL data has the strength of detecting possibly informal activities and their spatial patterns, disclosing regional governance differences (Proville, 2017).
Most commonly NL are interpreted as a sign of modernization, urbanization, security, economic development and technological innovation, however it is necessary to regard the downsides of out light emissions. Artificial light pollution is altering species circadian rhythm, as well as the human one. _Artificial light is used in modern societies to extend activity hours  i.e. working hours. Thus, NL could also indicate regional labor protection measurements._ Also it raises costs for 

"[...] usefulness of nighttime lights as proxy depends on optimal weight of proxy measure [...]" ([Chen, 2016](https://doi.org/10.1007/978-3-319-22810-5_15))
