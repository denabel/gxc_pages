...
flowchart TD
    ...
    C --> D[Disaggregation: matching GEOCON and NLs reoslution]
...
```{mermaid}
flowchart TD
    A[Start] --> B[Data: cquisition of geo-coded Demographic and health Surveys from African counties]
...
```{mermaid}
flowchart TD
    A[Start] --> B[Data: India Lights Project API, DMSP-OLS NL data from the National Oceanic and Atmospheric Administration (NOAA), 2011 Census of India, village shapefiles from InfoMap]
    B --> C[Pre-Processing: “stable light” to reduce noise, define centroids of villages]
    ...
    D --> E[Correlation analysis: correlation coefficient (Pearson’s r) between NL and Census data]
    E --> F[Regression & Evaluation: multivariate regression accuracy of modelling ground data, testing for non-linearity]
    F --> G[End]
...
```{mermaid}
flowchart TD
    A[Start] --> B[Data: annual composites of nighttime light data, Thematic World Borders Dataset for administrative boundaries (e.g., nation-states, territories), Socio-economic data sourced from World Bank (GDP, poverty headcount), UN, Eurostat, and national censuses (population), energy consumption (EIA) and emissions (CDIAC, EDGAR)]
    B --> C[Pre-Processing: thresholding (calculation of different light metrics, including “area lit”, “sum of lights,” “lights index,” and raw DN values), distinguish significance]
    C --> D[Data Integration: bilinear interpolation linking NLs data and centroids, calculating DN values in 2, 3, 5 km buffer around centroid]
    D --> E[Correlation analysis: Pearsons's R for correlation of NLss and socio-economic variables, regression models to analyse spatial patterns ]
    E --> F[Interpretation & Application: NLs as a proxy for socio-economic data in countries with poor statistical coverage, spatial variability in night-time lights]
    F --> G[End]
